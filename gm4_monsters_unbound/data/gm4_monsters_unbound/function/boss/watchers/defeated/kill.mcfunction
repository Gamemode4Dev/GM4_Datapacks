...
execute as @e[tag=gm4_mu_boss.watcher] if score @s gm4_mu_boss.id = $running_id gm4_mu_boss.id run kill @s

# if main entity run extra stuff
execute if entity @s[tag=gm4_mu_boss.watcher.main] run function gm4_monsters_unbound:boss/watchers/defeated/kill_main
