...
scoreboard players set @s[type=#gm4_survival_refightalized:skeleton_types] gm4_mu_arrow_fire_delay 3

data modify entity @s Motion set from storage gm4_monsters_unbound:temp set.motion

tag @s add gm4_mu_elite
tag @s add gm4_mu_weak_arrow
tag @s add gm4_mu_split_entity
function gm4_monsters_unbound:mob/init/calc_difficulty_base
