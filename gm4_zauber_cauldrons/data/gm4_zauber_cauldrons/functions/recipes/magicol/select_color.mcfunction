...
for color_data in ctx.meta['magicol_colors']:
    execute unless score $recipe_success gm4_zc_data matches 1.. if data storage gm4_zauber_cauldrons:temp/cauldron/ingredients items[{count:1,tag:{gm4_zauber_cauldrons:{item:color_data['ingredient_item_id']}}}] run function f"gm4_zauber_cauldrons:recipes/magicol/{color_data['color']}"
