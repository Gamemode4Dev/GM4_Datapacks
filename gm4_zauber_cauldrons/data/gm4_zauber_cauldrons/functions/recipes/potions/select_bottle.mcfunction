...
for bottle_data in ctx.meta['potion_bottles']:
    execute unless score $recipe_success gm4_zc_data matches 1.. if data storage gm4_zauber_cauldrons:temp/cauldron/ingredients items[{count:1,tag:{gm4_zauber_cauldrons:{item:bottle_data['item_id']}}}] run function f"gm4_zauber_cauldrons:recipes/potions/{bottle_data['bottle']}/check_liquid"
