...
for flower_data in ctx.meta['flower_types']:
    execute if score f"${flower_data['flower']}" gm4_zc_flowers matches 1 if data storage gm4_zauber_cauldrons:temp/cauldron/ingredients items[{count:1,tag:{gm4_zauber_cauldrons:{item:flower_data['item_id']}}}] run scoreboard players set $cancel_recipe gm4_zc_data 1
