# reconstruct in which way the player has interacted with the cauldron to get it into its minecraft:water_cauldron[level=1] state based on the previous state of the cauldron.
# @s = cauldron that was interacted with
# at position of cauldron, align xyz
# run from gm4_zauber_cauldrons:cauldron/liquid/level/1/ray

tellraw @a {"text":"Found targeted Zauber Cauldron (level=1).","color":"gray"}
...
execute if score $previous_fill_level gm4_zc_data matches 2 run function gm4_zauber_cauldrons:cauldron/liquid/level/1/item_used/glass_bottle
