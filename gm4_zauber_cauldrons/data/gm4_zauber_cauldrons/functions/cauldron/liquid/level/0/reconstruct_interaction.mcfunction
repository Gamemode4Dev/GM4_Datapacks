# reconstruct in which way the player has interacted with the cauldron to get it into its minecraft:cauldron (level=0) state based on the previous state of the cauldron.
# @s = cauldron that was emptied
# at position of cauldron, align xyz
# run from gm4_zauber_cauldrons:cauldron/liquid/level/0/ray

tellraw @a {"text":"Found targeted Zauber Cauldron. (level=NaN)","color":"gray"}
...
execute if score $previous_fill_level gm4_zc_data matches 3 run function gm4_zauber_cauldrons:cauldron/liquid/level/0/item_used/bucket

# if the cauldron was at level=1 previously a glass bottle must have been used on the cauldron
execute if score $previous_fill_level gm4_zc_data matches 1 run function gm4_zauber_cauldrons:cauldron/liquid/level/0/item_used/glass_bottle
