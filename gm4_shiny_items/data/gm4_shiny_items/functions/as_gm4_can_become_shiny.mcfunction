...
execute run data modify entity @s Item.tag.HideFlags set value 44

# Condition for item to loose tag "gm4_can_become_shiny"
execute unless entity @e[type=minecraft:experience_orb,distance=..2] run data remove entity @s Item.tag.HideFlags
...
execute unless entity @s[scores={gm4_absorbed_xp=0..}] run scoreboard players set @s gm4_absorbed_xp 0
