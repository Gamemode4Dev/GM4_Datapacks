...
# Blacklist Items
execute at @a as @e[type=item,distance=..20] at @s run function gm4_shiny_items:blacklist_items

# Condition for item to get tag "gm4_can_become_shiny"
execute as @e[type=minecraft:item,tag=!gm4_shiny_blacklist] at @s if entity @a[distance=..20] if entity @e[type=minecraft:experience_orb,distance=..2,tag=!gm4_shiny_blacklist] unless entity @e[distance=..2,nbt={Item:{tag:{HideFlags:44}}}] run tag @s add gm4_can_become_shiny
...
# Show Particle for any item that has XP absorbed
