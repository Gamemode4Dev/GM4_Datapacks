...
execute if score found_item_on_anvil gm4_bee_data matches 1 as @e[type=item,tag=gm4_wax_on_anvil] at @s if block ~ ~ ~ minecraft:moving_piston align xyz if entity @e[type=item,dx=0,nbt={Item:{tag:{Enchantments:[{}]}}}] run function gm4_ambrosia:beeswax/prepare_crafting
