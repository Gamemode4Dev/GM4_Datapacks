...
execute at @e[type=area_effect_cloud,tag=gm4_lt_util_block,tag=gm4_ltu_cauldron,dx=0,limit=4] positioned ^ ^-1 ^1 if block ~ ~ ~ cauldron[level=0] run function gm4_standard_liquids:util/cauldron
...
scoreboard players operation @s gm4_lt_util < @s gm4_lt_value
execute at @e[type=area_effect_cloud,tag=gm4_lt_util_block,tag=gm4_ltu_furnace,dx=0,limit=4] positioned ^ ^-1 ^1 if block ~ ~ ~ furnace[lit=false]{Items:[{Slot:0b}]} unless block ~ ~ ~ furnace{Items:[{Slot:1b}]} run function gm4_standard_liquids:util/furnace_start
scoreboard players reset @s gm4_lt_util
