...
execute store success score $dropped_item gm4_machine_data as @e[type=item,nbt={Item:{tag:{display:{Name:'{"italic":false,"translate":"gui.gm4.hotswapper","fallback":"Customize Hotbars"}'}}}},distance=..2,limit=1,sort=nearest] run data modify entity @s Item.tag merge from storage gm4_hotswap_hotbars:temp item_data
