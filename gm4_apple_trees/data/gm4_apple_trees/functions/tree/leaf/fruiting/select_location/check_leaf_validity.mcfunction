...
execute store success score $created_apple_leaf gm4_apple_data align xyz if block ~ ~1 ~ minecraft:oak_leaves[persistent=false] if block ~ ~ ~ #gm4_apple_trees:tree_replaceable unless entity @e[type=armor_stand,tag=gm4_apple_leaf,dx=0,limit=1] run function gm4_apple_trees:tree/leaf/fruiting/generate
