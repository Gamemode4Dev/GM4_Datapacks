execute if score gm4 load.status matches 1 if score gm4_fruiting_trees load.status matches 1 run scoreboard players set gm4_apple_trees load.status 1
...
execute unless score gm4_fruiting_trees load.status matches 1 run data modify storage gm4:log queue append value {type:"missing",module:"Apple Trees",require:"Fruiting Trees"}
