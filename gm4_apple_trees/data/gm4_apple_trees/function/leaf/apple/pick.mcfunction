...
# update item display
execute if predicate gm4_apple_trees:empty_or_apple_mainhand unless data entity @s SelectedItem.components run tag @e[type=item_display,tag=gm4_apple_taken,distance=..10] add gm4_skip_item_drop
...
execute if items entity @s weapon.mainhand minecraft:apple[count={min:1,max:63}] unless data entity @s SelectedItem.components run item modify entity @s weapon.mainhand {function:"minecraft:set_count",count:1,add:1}
