...
# set score h ID to lantern's
execute store result score @s gm4_sf_h_id run data get entity @s SelectedItem.tag.gm4_soul_fragments.id
execute if entity @s[scores={gm4_sf_h_id=0}] store result score @s gm4_sf_h_id run data get entity @s Inventory[{Slot:-106b}].tag.gm4_soul_fragments.id
...
scoreboard players operation @e[type=minecraft:area_effect_cloud,tag=gm4_sf_new_ray] gm4_sf_id = @s gm4_sf_h_id
...
scoreboard players reset @s gm4_sf_h_id
