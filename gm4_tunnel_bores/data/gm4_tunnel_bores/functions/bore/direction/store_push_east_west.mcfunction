...
# get force values
execute store result score PushX gm4_bore_data run data get entity @s PushX

# check force direction (+/-) and store to scoreboard
execute if score PushX gm4_bore_data matches 1.. run scoreboard players set @s gm4_bore_data -90
execute if score PushX gm4_bore_data matches ..-1 run scoreboard players set @s gm4_bore_data 90
