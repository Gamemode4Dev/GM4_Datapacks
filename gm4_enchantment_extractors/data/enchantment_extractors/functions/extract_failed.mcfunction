...
execute if score result gm4_ench_random matches 80..89 run summon vex ~ ~-1 ~ {CustomName:'{"translate":"%1$s",with: ["Congealed Enchantment",{"translate":"entity.gm4.congealed_enchantment"}]}',CustomNameVisible:0,LifeTicks:100,Attributes:[{Name:generic.attackDamage,Base:2}],Health:4.0,Motion:[0.0,0.35,0.0]}
execute if score result gm4_ench_random matches 90.. run playsound entity.evoker.prepare_attack block @a[distance=..8] ~ ~ ~ 1.3 .6
execute if score result gm4_ench_random matches 90.. run summon vex ~ ~-1 ~ {CustomName:'{"translate":"%1$s",with: ["Living Enchantment",{"translate":"entity.gm4.living_enchantment"}]}',CustomNameVisible:0,HandItems:[{id:golden_sword,Count:1},{}],HandDropChances:[0.0f,0.0f],Motion:[0.0,0.25,0.0]}
