...
# [Debug]
execute if score $palm_spreader_loop gm4_gv_tree_gen = $adjusted_palm_amount gm4_gv_tree_gen run data modify storage gm4_garden_variety:debug/generation path append value ""
...
execute store result entity @s Rotation[0] float 1 run scoreboard players get $palm_rotation_modified gm4_gv_tree_gen

# set scores for the next part of generation
scoreboard players operation $palm_layer_loop gm4_gv_tree_gen = $palm_layers gm4_gv_tree_gen
...
execute if score $palm_spreader_loop gm4_gv_tree_gen matches 0 run kill @s
execute if score $palm_spreader_loop gm4_gv_tree_gen matches 1.. run function gm4_garden_variety:mechanics/tree_generation/palm_tree/palm_spreader
