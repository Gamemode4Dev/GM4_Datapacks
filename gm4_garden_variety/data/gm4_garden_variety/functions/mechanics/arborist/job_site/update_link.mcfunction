...
execute unless entity @s[tag=gm4_gv_disable_job_site] as @e[type=villager,tag=gm4_gv_arborist,distance=..10] run function gm4_garden_variety:mechanics/arborist/job_site/check_villager
...
execute if entity @s[tag=!gm4_gv_disable_job_site] if score @s gm4_gv_arborist matches 1.. as @e[tag=!gm4_gv_has_job_site,type=villager,distance=..10,limit=1,sort=nearest] store success score $link_villager_success gm4_gv_job_id run function gm4_garden_variety:mechanics/arborist/job_site/link_villager
