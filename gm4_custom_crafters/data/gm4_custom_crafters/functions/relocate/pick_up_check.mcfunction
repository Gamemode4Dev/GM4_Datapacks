...
execute if score valid_machine gm4_rl_data matches 0 store success score valid_machine gm4_rl_data if block ~ ~ ~ dropper{Items:[]} align xyz run tag @e[type=armor_stand,tag=gm4_custom_crafter,tag=!gm4_master_crafting,dx=0,dy=0,dz=0,limit=1] add gm4_rl_selected
