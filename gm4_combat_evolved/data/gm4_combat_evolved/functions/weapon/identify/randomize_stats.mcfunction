...
loot replace block 29999998 1 7134 container.2 loot gm4_combat_evolved:randomizer/damage
loot replace block 29999998 1 7134 container.3 loot gm4_combat_evolved:randomizer/speed
loot replace block 29999998 1 7134 container.4 loot gm4_combat_evolved:randomizer/atkspeed
...
# attack damage 0.5 - 2
execute store result score $damage gm4_ce_data run data get storage gm4_combat_evolved:temp Items[{Slot:2b}].Count
...
# movement speed 5 - 20
execute store result score $speed gm4_ce_data run data get storage gm4_combat_evolved:temp Items[{Slot:3b}].Count
...
# attack speed 2 - 5
execute store result score $atkspeed gm4_ce_data run data get storage gm4_combat_evolved:temp Items[{Slot:4b}].Count
...
# random value 1 - 3
execute store result score $random_value gm4_ce_data run data get storage gm4_combat_evolved:temp Items[{Slot:5b}].Count

# calculate new damage in order to replace the attribute
execute store result score $damage_stored gm4_ce_data run data get storage gm4_combat_evolved:temp tag.gm4_combat_evolved.damage
# add sharpness levels to damage
execute store result score $sharpness gm4_ce_data run data get storage gm4_combat_evolved:temp tag.Enchantments[{id:"minecraft:sharpness"}].lvl
...
scoreboard players operation $damage_double gm4_ce_data = $damage_stored_10 gm4_ce_data
scoreboard players operation $damage_double gm4_ce_data *= #2 gm4_ce_data

# calculate new attack speed in order to replace the attribute
execute store result score $atkspeed_stored gm4_ce_data run data get storage gm4_combat_evolved:temp tag.gm4_combat_evolved.atkspeed
...
execute store result storage gm4_combat_evolved:temp random_value int 1 run scoreboard players get $random_value gm4_ce_data
