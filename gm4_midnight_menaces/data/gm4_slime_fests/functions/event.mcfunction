...
# resurrecting slimes
execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:slime_ball"},OnGround:1b}] run function gm4_slime_fests:mark_slime_ball
execute as @e[type=minecraft:item,scores={gm4_reslimify=8..}] at @s run function gm4_slime_fests:spawn_slime

# slime takeover
execute as @e[type=#gm4_slime_fests:slime_convertable,nbt={PersistenceRequired:0b}] at @s run function gm4_slime_fests:slime_transform

# merging slimes
scoreboard players reset @e[type=minecraft:slime] gm4_menace_data
execute at @e[type=minecraft:slime] run scoreboard players add @e[type=minecraft:slime,distance=0.1..5] gm4_menace_data 1
