# @s = mould requesting a thorium band
# run from set_mould

#tell expansions to call representative AEC
function #metallurgy:random_band_init/thorium

#select and tag a random representative AEC
tag @e[type=area_effect_cloud,distance=..0.5,tag=gm4_ml_raw_thorium_band,sort=random,limit=1] add gm4_ml_selected_shamir
...
function #metallurgy:summon_band/thorium
