...
# add primary metal
scoreboard players operation @s gm4_ml_ore_th += ore_amount gm4_ml_data

# add secondary metal
execute store result score ore_amount gm4_ml_data run data get storage gm4_metallurgy:temp/item/ore gm4_metallurgy.ore.amount[1]
scoreboard players operation @s gm4_ml_ore_cu += ore_amount gm4_ml_data
...
kill @e[type=item,tag=gm4_ml_in_animation,dx=0,dz=0,limit=1,nbt={Item:{Count:1b,tag:{gm4_metallurgy:{ore:{type:"thorium_brass"}}}},OnGround:1b}]
