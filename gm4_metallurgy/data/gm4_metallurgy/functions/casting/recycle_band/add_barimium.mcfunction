#@s = sand ring that needs to accept metals from melted band
# run from melt_band

#add score to sand ring
tag @s add gm4_contains_metal
data merge entity @s {ArmorItems:[{},{},{},{id:"minecraft:player_head",Count:1,tag:{CustomModelData:26,SkullOwner:{Id:[I;140191582,-1151712444,-1271433465,-1758951935],Properties:{textures:[{Value:""}]}}}}]}
scoreboard players add @s gm4_ml_ore_al 9
scoreboard players add @s gm4_ml_ore_ba 3
particle block stone ~.45 ~0.1 ~.65 .1 .1 .1 0 9
particle block prismarine_bricks ~.45 ~0.1 ~.65 .1 .1 .1 0 2
playsound item.bucket.empty_lava block @a[distance=..8] ~ ~ ~ .25 1.5
setblock ~ ~1 ~ minecraft:air
summon item ~ ~.6 ~ {Item:{id:"minecraft:obsidian",Count:1b,tag:{display:{Lore:['{"translate":"%1$s%3427655$s","with":["Slightly Damaged Obsidian",{"translate":"item.gm4.slightly_damaged_obsidian"}],"italic":false}']}}}}
advancement grant @a[distance=..5,gamemode=!spectator] only gm4:metallurgy_cast
