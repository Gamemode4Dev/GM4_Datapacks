#@s = obsidian cast with a levity shamir on it
#called from initiate_transfer

#check whether the other item is valid
scoreboard players set valid_tool gm4_ml_data 0
execute as @e[type=item,dx=0,limit=1,nbt=!{Item:{tag:{gm4_metallurgy:{has_shamir:1b}}}}] run function levity_shamir:transfer_shamir/check_tool_validity
