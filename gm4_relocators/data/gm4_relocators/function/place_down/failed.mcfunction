...
data modify storage gm4_relocators:temp lore set from entity @s SelectedItem.components."minecraft:lore"[0]
execute at @s[gamemode=!creative,gamemode=!spectator] run loot spawn ~ ~.3 ~ loot gm4_relocators:items/relocator_full
data merge entity @e[type=item,distance=..7,nbt={Age:0s,Item:{id:"minecraft:player_head",count:1,components:{"minecraft:custom_data":{gm4_machines:{id:"relocator_full"}}}}},limit=1] {PickupDelay:0}
