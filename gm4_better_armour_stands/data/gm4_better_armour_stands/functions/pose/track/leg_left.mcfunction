...
execute if entity @s[tag=gm4_bas_invert] store result storage gm4_better_armour_stands:temp Pose.LeftLeg[0] float 0.01 run scoreboard players add $joint_rot_x gm4_bas_data 9000
execute unless entity @s[tag=gm4_bas_invert] store result storage gm4_better_armour_stands:temp Pose.LeftLeg[0] float 0.01 run scoreboard players remove $joint_rot_x gm4_bas_data 9000
