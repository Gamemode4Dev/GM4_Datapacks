...
execute if data storage gm4:log log{type:"install"} run tellraw @a[tag=gm4_show_log] [{nbt:"log.module",storage:"gm4:log",plain:true},{text:" Installed!",color:"white"}]
execute if data storage gm4:log log{type:"missing"} run tellraw @a[tag=gm4_show_log] [{nbt:"log.module",storage:"gm4:log",plain:true,color:"red"},{text:" is disabled because ",color:"red"},{nbt:"log.require",storage:"gm4:log",plain:true,color:"red"},{text:" is not installed."}]
