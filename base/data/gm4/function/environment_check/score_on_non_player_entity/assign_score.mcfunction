...
execute if score @s gm4_data matches 1 run data modify storage gm4:log environment_checks[{ecid:"gm4:score_on_non_player_entity"}].result set value {passed: 1b, probable_cause: ""}

# (optional) if no score is present, provide a probable cause message to the user
execute unless score @s gm4_data matches 1 run data modify storage gm4:log environment_checks[{ecid:"gm4:score_on_non_player_entity"}].result set value {passed: 0b, probable_cause: "This may be caused by the Paper/Spigot setting 'scoreboards.allow-non-player-entities-on-scoreboards=false'."}
