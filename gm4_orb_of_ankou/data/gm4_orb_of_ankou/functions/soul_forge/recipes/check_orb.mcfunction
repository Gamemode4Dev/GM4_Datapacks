...
execute as @e[type=armor_stand,tag=gm4_soul_forge,limit=1,sort=nearest,distance=..1] store success score has_orb gm4_oa_forge if entity @s[tag=gm4_oa_has_orb]

# check pneuma count of orb
execute store result score pneuma_count gm4_oa_forge run data get entity @s Item.tag.gm4_orb_of_ankou.pneumas
...
execute if score has_orb gm4_oa_forge matches 1 store result score orb_count gm4_oa_forge run data get entity @s Item.Count
