...
execute as @e[type=armor_stand,tag=gm4_soul_forge,limit=1,sort=nearest,distance=..1] store success score has_shard gm4_oa_forge if entity @s[tag=gm4_oa_has_shard]

execute if score has_shard gm4_oa_forge matches 0 run function gm4_orb_of_ankou:soul_forge/recipes/add_shard
execute if score has_shard gm4_oa_forge matches 1 store result score shard_count gm4_oa_forge run data get entity @s Item.Count
