...
execute store result score $curer gm4_mob_cu_data run data get storage gm4_mob_curing:temp Gossips[0].Target[3] 0.001
execute as @a[distance=..10] store result score @s gm4_mob_cu_data run data get entity @s UUID[3] 0.001
execute as @a[distance=..10] if score @s gm4_mob_cu_data = $curer gm4_mob_cu_data run advancement grant @s only gm4:mob_curing
