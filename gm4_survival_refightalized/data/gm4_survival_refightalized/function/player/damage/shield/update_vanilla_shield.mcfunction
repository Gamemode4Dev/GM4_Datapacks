...
# offhand
execute if entity @s[advancements={gm4_survival_refightalized:using_vanilla_shield={offhand=true}}] run item modify entity @s weapon.offhand { \
  type:"set_components", \
  components: { \
    "custom_data": { \
      "gm4_survival_refightalized":{ \
        "shield": {"parry_ticks": 5} \
      } \
    }, \
    "minecraft:blocks_attacks": { \
      ...
      "item_damage": {"base": 1.0, "factor": 1.0, "threshold": 3.0} \
    } \
  } \
}

# mainhand
execute if entity @s[advancements={gm4_survival_refightalized:using_vanilla_shield={mainhand=true}}] run item modify entity @s weapon.mainhand { \
  type:"set_components", \
  components:{ \
    "custom_data":{ \
      "gm4_survival_refightalized":{ \
