...
# regenerate HP up to 50% (can be altered by expansions)
function gm4_survival_refightalized:player/health/calculate_hp
execute if score @s gm4_sr_stat.health_percentage < @s gm4_sr_stat.regen_to_percentage \
    unless score @s gm4_sr_health.regeneration_timer matches 1.. \
    unless score @s gm4_sr_stat.hunger matches ..6 run function gm4_survival_refightalized:player/health/regeneration/combat_health
