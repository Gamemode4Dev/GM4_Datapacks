...
execute as @a[scores={gm4_sr_stat.sleep_in_bed=1..},gamemode=!spectator] run function gm4_survival_refightalized:player/detect_sleep
...
execute if score $natural_regen gm4_sr_config matches 0 as @a[gamemode=!spectator] unless score @s gm4_sr_health.regeneration_timer matches 1.. unless score @s gm4_sr_stat.hunger matches ..6 run function gm4_survival_refightalized:player/health/regen_combat_health
execute if score $natural_regen gm4_sr_config matches 0 as @a[scores={gm4_sr_health.quick_regeneration_health=1..},gamemode=!spectator] unless score @s gm4_sr_health.quick_regeneration_timer matches 1.. run function gm4_survival_refightalized:player/health/regen_fast_health

# armor recharge timer
execute as @a[gamemode=!spectator,tag=gm4_sr_armor.reduction] run function gm4_survival_refightalized:player/armor/recharge_timer
...
execute as @a[gamemode=!spectator,scores={gm4_sr_health.regeneration_timer=1..}] run function gm4_survival_refightalized:player/health/regeneration_timer
