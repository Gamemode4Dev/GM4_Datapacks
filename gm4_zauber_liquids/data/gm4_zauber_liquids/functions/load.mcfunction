execute if score gm4 load.status matches 1 if score gm4_liquid_tanks load.status matches 1 if score gm4_zauber_cauldrons load.status matches 1 run scoreboard players set gm4_zauber_liquids load.status 1
...
execute unless score gm4_liquid_tanks load.status matches 1 run data modify storage gm4:log queue append value {type:"missing",module:"Zauber Liquids",require:"Liquid Tanks"}
