execute unless data entity @s SelectedItem.components."minecraft:custom_data".gm4_combat_expanded{identified:0}.damage run loot replace entity @s weapon.mainhand loot gm4_ce_base:weapon/sword/iron
data modify storage gm4_combat_expanded:temp components set from entity @s SelectedItem.components

function gm4_ce_base:identification/weapon/randomize_stats
$loot replace block 29999998 1 7134 container.1 loot gm4_ce_base:weapon/identification/modifier/$(id)
data modify storage gm4_combat_expanded:temp new_components merge from block 29999998 1 7134 Items[{Slot:1b}].components

# fix components
function gm4_ce_base:identification/weapon/fix_components
...
execute if score $current_sharpness gm4_ce_data matches 1.. run function gm4_ce_base:weapon/update_sharpness
