...
# check for mob cap
execute at @p[gamemode=!spectator] store result score $spore_zombie_count gm4_ce_data if entity @e[type=#gm4_ce_base:zombie_types,tag=gm4_ce_spore_mob,distance=..32]
...
execute as @e[tag=gm4_ce_spore_mob,distance=..1] at @s run function gm4_ce_base:mob/process/spore/set_generation
