...
execute if score $delay_damage gm4_ce_data < @s gm4_ce_health.current run function gm4_ce_base:weapon/modifier/delay/explode_player_eval with storage gm4_combat_expanded:temp damage_player

# kill player if health reached 0
execute if score $delay_damage gm4_ce_data >= s gm4_ce_health.current run function gm4_ce_base:weapon/modifier/delay/player_death
