...
execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:saddle"}}] at @s if block ~ ~ ~ #stairs[half=bottom] positioned ~ ~-.1 ~ align xyz unless entity @e[type=pig,tag=gm4_chairs,dx=0,dz=0,dy=0] at @s run function gm4_chairs:saddle

#kill pig/saddle if stair block broken
execute as @e[type=pig,tag=gm4_chairs] at @s unless block ~ ~1 ~ #stairs run function gm4_chairs:kill
