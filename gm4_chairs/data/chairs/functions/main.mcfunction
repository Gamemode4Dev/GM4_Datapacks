...
execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:saddle"}}] at @s if block ~ ~ ~ #minecraft:stairs[half=bottom] positioned ~ ~-.1 ~ align xyz unless entity @e[type=pig,tag=chairs,dx=0,dz=0,dy=0] at @s run function chairs:saddle
