...
execute as @e[type=wolf,tag=gm4_ce_wolf] if score @s gm4_ce_id = $curr_id gm4_ce_id if score @s gm4_ce_data = $slot gm4_ce_data run function gm4_combat_expanded:armor/modifiers/type/canine/wolf_checked

# summon new wolf if there is none
execute if score $wolf gm4_ce_data matches 0 at @s run function gm4_combat_expanded:armor/modifiers/type/canine/wolf_spawn
