...
# Phase 2 - Inprove and unlock additional attacks, most attacks will run faster
...
# Phase 3 - Spawn a sentinel with 250 hp that can use Phase 1 attacks
...
# Phase 4 - Gain additional resistance, boss attacks much faster
...
## EXPLODE MINIONS
# Phase 1: -
# Phase 2: Guardian commands all living minions to explode, one after another
# Phase 3: dissapear while minions are exploding by teleporting 1000 blocks up (100%)
...
execute as @e[type=#gm4_combat_expanded:zombie_types,tag=gm4_ce_boss.minion] if score @s gm4_ce_boss.id = $running_id gm4_ce_boss.id at @s if entity @p[gamemode=!spectator,gamemode=!creative,distance=..6] run scoreboard players set $explode_minions_valid gm4_ce_boss 1
execute unless score @s[scores={gm4_ce_boss.attack_id=0,gm4_ce_boss.phase=2..}] gm4_ce_boss.last_attack matches 6 if score $explode_minions_valid gm4_ce_boss matches 1 if predicate gm4_combat_expanded:technical/chance/boss/explode_minions run scoreboard players set @s gm4_ce_boss.attack_id 6
...
# Phase 2: spawn 4 minions instead
# Phase 3: minions are spawned faster
# Phase 4: raise blocks from the ground near players that also spawn mobs (2 per player)
...
execute unless score @s[scores={gm4_ce_boss.attack_id=0,gm4_ce_boss.phase=2..}] gm4_ce_boss.last_attack matches 7 if entity @p[gamemode=!spectator,gamemode=!creative,distance=..5.25] if predicate gm4_combat_expanded:technical/chance/boss/aura_attack run scoreboard players set @s gm4_ce_boss.attack_id 7

## WARP
# Phase 1: Guardian charges for a time, then warps away to a location nearby a player, can be above or next to them
...
execute unless score @s gm4_ce_boss.attack_id matches 1.. run scoreboard players set @s gm4_ce_boss.attack_id 30
