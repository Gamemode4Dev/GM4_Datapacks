...
execute unless score $curr_health gm4_ce_data = @s gm4_ce_data run function gm4_combat_expanded:mob/process/elite/gargantuan/update_stats

# no nearby players
execute unless score @s gm4_ce_timer matches 1.. unless entity @a[gamemode=!spectator,distance=..7] run return 0
# otherwise 10% chance to start attack
execute unless score @s gm4_ce_timer matches 1.. if entity @a[gamemode=!spectator,distance=..7] unless predicate gm4_combat_expanded:technical/chance/gargantuan_ground_smash run return 0
...
execute if score @s gm4_ce_timer matches 2..4 rotated 0 0 run function gm4_combat_expanded:mob/process/elite/gargantuan/particle_ring
...
attribute @s[scores={gm4_ce_timer=1}] generic.follow_range modifier add gm4_combat_expanded:elite_buff.giant.charging -1 add_multiplied_total
...
execute if score @s gm4_ce_timer matches 5 run function gm4_combat_expanded:mob/process/elite/gargantuan/activate
