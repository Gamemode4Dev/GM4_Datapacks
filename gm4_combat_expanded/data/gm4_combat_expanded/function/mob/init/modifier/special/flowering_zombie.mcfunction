...
execute store result entity @s equipment.head.components."minecraft:custom_data".gm4_ce_spore.generation int 1 run scoreboard players add @s gm4_ce_generation 1

# let generation determine dropchance of spore
scoreboard players set $spore_dropchange gm4_ce_data 11
execute store result entity @s drop_chances.head float 0.1 run scoreboard players operation $spore_dropchange gm4_ce_data -= @s gm4_ce_generation
