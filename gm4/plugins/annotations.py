...
class AnnotationFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        match = re.match(r"(.+):(\d+):(\d+)", getattr(record, "annotate", "")) # extract source filename and location from mecha annotation
        if match:
            filename, line, col = match.groups()
            return f"::{level} file={filename},line={line},col={col},title={record.name}::{record.name} {expl}"

        match = re.match(r"(.+)/File (.+), line (\d+), column (\d+)", getattr(record, "annotate", "")) # if file generated by beet, still include resource location
        if match:
            ...
            expl = expl.replace("%0A", f"{location_hint}%0A", 1) # put new hint text before the rest of the log message
