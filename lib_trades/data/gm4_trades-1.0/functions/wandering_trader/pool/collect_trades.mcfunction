...
data modify storage gm4_trades-1.0:temp/wandering_trader/comparison pool set from storage gm4_trades-1.0:temp/wandering_trader/current_tradepool pool
execute if data entity @s ArmorItems[0].tag.gm4_trades.pool store success score $pools_differ gm4_trades_data run data modify storage gm4_trades-1.0:temp/wandering_trader/comparison pool set from entity @s ArmorItems[0].tag.gm4_trades.pool
data remove storage gm4_trades-1.0:temp/wandering_trader/comparison pool
...
execute as @e[type=armor_stand,tag=gm4_trade_option,tag=!gm4_processed_trade_option,limit=1,sort=arbitrary] run function gm4_trades-1.0:wandering_trader/pool/collect_trades
