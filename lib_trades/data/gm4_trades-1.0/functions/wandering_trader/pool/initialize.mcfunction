...
data modify storage gm4_trades-1.0:temp/wandering_trader/current_tradepool pool set from entity @s DecorItem.tag.gm4_trades.pool

# compare to next trade option's pool
execute as @e[type=trader_llama,tag=gm4_trade_option,tag=!gm4_pooled_trade_option,limit=1,sort=arbitrary] run function gm4_trades-1.0:wandering_trader/pool/collect_trades
