...
data modify storage gm4_trades-1.0:temp/wandering_trader/current_tradepool pool set from entity @s ArmorItems[0].tag.gm4_trades.pool

# compare to next trade option's pool
execute as @e[type=armor_stand,tag=gm4_trade_option,tag=!gm4_pooled_trade_option,limit=1,sort=arbitrary] run function gm4_trades-1.0:wandering_trader/pool/collect_trades

# clean up tag and storage (tag cleanup is very important here!)
tag @e[type=armor_stand,tag=gm4_trade_option] remove gm4_processed_trade_option
data remove storage gm4_trades-1.0:temp/wandering_trader/current_tradepool pool
