...
execute at @a[tag=gm4_orbis_disabled] positioned ~-8 ~ ~-8 as @e[type=area_effect_cloud,tag=gm4_chunk,tag=!gm4_generated,sort=nearest,limit=1,distance=..100] at @s if block ~ ~ ~ bedrock run function orbis:chunk/generate
...
execute as @a[tag=!gm4_orbis_disabled,nbt={Dimension:0}] at @s unless block ~ 0 ~ barrier run function orbis:chunk/init
