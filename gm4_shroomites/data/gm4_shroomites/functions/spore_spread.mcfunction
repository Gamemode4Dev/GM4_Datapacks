...
execute if entity @s[nbt={Age:2560},scores={gm4_sporeprint=6..}] if block ~ ~-1 ~ #shroomites:spore_spreadable unless entity @e[tag=gm4_shroomite_shroom,type=area_effect_cloud,distance=..4.5] if block ~ ~ ~ #shroomites:shroom_rootable run function gm4_shroomites:shroom_spawn
