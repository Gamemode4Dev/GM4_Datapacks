...
scoreboard players operation $damage_health gm4_ce_data -= $link_health gm4_ce_data
scoreboard players operation @s gm4_ce_healstore -= $damage_health gm4_ce_data

# pause regen next tick if damaged
tag @s[scores={gm4_ce_healstore=..-1}] add gm4_ce_pause_nat_regen

# change player health
execute if score @s gm4_ce_healstore matches 1.. run function gm4_combat_expanded_armor:player/heal/heal_calc
execute if score @s gm4_ce_healstore matches ..-1 run function gm4_combat_expanded_armor:player/damage/activate
