...
execute unless entity @e[type=block_display,tag=gm4_ce_beacon.process,distance=..12] run function gm4_combat_expanded_armor:armor/modifier/type/beacon/spawn

# check if player is sneaking
execute store result score $player_sneaking gm4_ce_data if predicate gm4_combat_expanded_armor:technical/crouching

# process the players beacon
execute as @e[type=block_display,tag=gm4_ce_beacon.process,distance=..12] run function gm4_combat_expanded_armor:armor/modifier/type/beacon/process
