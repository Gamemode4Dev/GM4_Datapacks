...
# run from mechanincs/interactions/light_candle/ray_init
...
execute if block ~ ~ ~ minecraft:lily_pad align xyz positioned ~.5 ~.02 ~.5 \
  ...
  run return run function gm4_lively_lily_pads:mechanics/interactions/light_candle/modify

# loop
scoreboard players remove $ray gm4_llp.data 1
execute if score $ray gm4_llp.data matches 0.. positioned ^ ^ ^.1 run function gm4_lively_lily_pads:mechanics/interactions/light_candle/ray
