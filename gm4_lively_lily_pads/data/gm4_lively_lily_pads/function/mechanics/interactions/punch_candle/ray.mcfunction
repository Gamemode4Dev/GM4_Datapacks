...
# @s = player who wants to break a candle
# at @s anchored eyes, moving forward
# run from mechanincs/interactions/punch_candle/hit_rcd and self
...
execute if block ~ ~ ~ minecraft:lily_pad align xyz positioned ~.5 ~.02 ~.5 \
  ...
  run return run function gm4_lively_lily_pads:mechanics/cleanup/kill

# loop
scoreboard players remove $ray gm4_llp.data 1
execute if score $ray gm4_llp.data matches 0.. positioned ^ ^ ^.5 run function gm4_lively_lily_pads:mechanics/interactions/punch_candle/ray
