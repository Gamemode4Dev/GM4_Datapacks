...
execute store result score $killed_player_count gm4_aa_data run tag @a[tag=gm4_aa_link.process,scores={gm4_sr_stat.current_health=..0}] add gm4_aa_link.killed
execute if score $killed_player_count gm4_aa_data matches 2.. run tag @r[tag=gm4_aa_link.killed] add gm4_aa_link.killed_2

execute as @a[tag=gm4_aa_link.process,tag=!gm4_aa_link.killed] run function gm4_augmented_armor:armor/augment/type/linked/process/death_message
